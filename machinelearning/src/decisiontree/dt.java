...
public class dt
{
    ...
    /* The class represents a data point consisting of numAttributes values of attributes */
    class DataPoint {
    ...
    /* The class represents a node in the decomposition tree.*/
    class TreeNode {
        public double entropy;
        public Vector data;
        public int decompositionAttribute;  //selected attribute
        public int decompositionValue;  //index of the node's children
    ...
    public double calculateEntropy(Vector data) {
        ...
        System.out.println("attribute:" + attribute + " domain:" + domains[attribute]);
        
        int numvalues = domains[attribute].size();  //domains[3] = [yes,no], num of kinds for some attribute
        //System.out.println("data size:" + numdata + " domains size:" + numvalues);

        double sum = 0;
        for (int i=0; i< numvalues; i++) {  //each domain for some attribute
            int count=0;
            for (int j=0; j< numdata; j++) { //data on each line
    ...
    public void decomposeNode(TreeNode node) {
        ...
        node.entropy = calculateEntropy(node.data);  //get the entropy H(D)
        if (node.entropy == 0)
            return;
    
        for (int i=0; i< numinputattributes; i++) { //num of attributes 
            int numvalues = domains[i].size();
            //System.out.println("domains[i]:" + domains[i] + ",size:" + numvalues);
            
            if ( alreadyUsedToDecompose(node, i) )
                continue;
        
            double averageentropy = 0;
            for (int j=0; j< numvalues; j++) { //num of domains for some attribute
                //get the subset of some attribute and some domain
                Vector subset = getSubset(node.data, i, j); //e.g. [slashdot, yes] and [google, no/yes]
            ...
            //choose the best attribute according to the entropy
            if (selected == false) {
                selected = true;
        ...
        //decompose the tree into numvalues children according the attribute 
    ...
    public int readData(String filename) throws Exception {
        ...
        while(true) {
            ...
            for (int i=0; i < numAttributes; i++) {
                //attributes[i] stores the index of domains[attribute]
                point.attributes[i] = getSymbolValue(i, tokenizer.nextToken());  //
                //System.out.print(i + ":" + point.attributes[i] + ";");
            }
            //System.out.println();
